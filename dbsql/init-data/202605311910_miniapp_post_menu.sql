...
WHERE p.`id` IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM `menus` WHERE `code` = 'platform:miniapp:post:list');
...
SET
  ...
  child.`routePath` = '/miniapp/posts',
  child.`componentPath` = 'miniapp/posts/page',
  child.`sortOrder` = 7,
  child.`isHidden` = 0,
  child.`isActive` = 1,
  child.`description` = '小程序发布信息审核管理'
WHERE child.`code` = 'platform:miniapp:post:list'
...
WHERE r.`scope` = 'platform'
  ...
  AND m.`code` = 'platform:miniapp:post:list';
