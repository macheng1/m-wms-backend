...
-- 前置条件：空库需先执行 init-schema.sql；旧库需先执行对应 schema.sql。
...
VALUES
  ...
  ('platform:tenant', 'platform', '租户管理', 'MENU', 0, '租户管理'),
  ...
  ('tenant:product', 'tenant', '产品管理', 'MENU', 0, '产品管理'),
  ...
  ('tenant:warehouse', 'tenant', '仓库管理', 'MENU', 0, '仓库管理'),
  ...
  ('tenant:inventory', 'tenant', '库存管理', 'MENU', 0, '库存管理'),
  ...
  ('tenant:order', 'tenant', '订单管理', 'MENU', 0, '订单管理'),
  ...
  ('tenant:portal', 'tenant', '官网管理', 'MENU', 0, '官网管理'),
