...
graph LR
    ...
    Middleware -->|JWT认证| Guard[守卫层]
    Guard -->|路由匹配| Controller[控制器层]
    Controller -->|参数验证| Service[服务层]
    Service -->|业务逻辑| Repository[数据访问层]
    Repository -->|SQL执行| Database[(MySQL数据库)]
    Database -->|返回结果| Repository
    Repository -->|实体数据| Service
    Service -->|DTO响应| Controller
...
| 层级 | 职责 |
|------|------|
| **客户端** | 发起 HTTP 请求到后端 API |
| **中间件** | 处理跨域、全局前缀等 |
| **守卫** | JWT 身份验证 |
| **控制器** | 接收请求、参数验证、返回响应 |
| **服务** | 处理业务逻辑 |
| **数据访问** | TypeORM Repository 操作数据库 |
| **数据库** | MySQL 数据持久化 |
